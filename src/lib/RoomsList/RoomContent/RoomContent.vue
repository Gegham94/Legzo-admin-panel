...
export default {
  ...
  computed: {
    ...
    getAgent() {
      const agentId = this.room.primary_user_id;
      const agent = this.room.users.find(user => user.id === agentId);
      return agent ? agent.name : null;
    },
    getMessageAuthor() {
      const msgAuthorId = this.room.lastMessage.from_user_id;
      const msgAuthor = this.room.users.find(user => user.id === msgAuthorId);
      return msgAuthor ? `${msgAuthor.name.slice(0, 2)}:` : null;
    },
    getEmoji() {
      const emojiCodes = ["&#128078", "&#128077"];
      const lastMessage = this.room.lastMessage.msg;
      for (const emoji of emojiCodes) {
        if (lastMessage.includes(emoji)) {
          return emoji;
        }
      }
      return null;
