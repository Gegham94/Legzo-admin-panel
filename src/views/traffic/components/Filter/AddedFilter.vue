...
import WebCrawlers from "./components/web-crawlers/WebCrawlers.vue";
...
const emit = defineEmits(["apply-filter", "send-filter"]);
...
<template>
  <div class="filter-wrapper">
    ...
    >
      ...
      <template v-if="filter.id === 'assigned_agent' && filter.used">
        <AssignedAgent
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'country_code' && filter.used">
        <Country
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'state' && filter.used">
        <State
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'returning_customer' && filter.used">
        <ReturningCustomer
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'number_of_visits' && filter.used">
        <NumberOfVisits
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'web_crawlers' && filter.used">
        <WebCrawlers
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'ip_address' && filter.used">
        <IpAddress
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'came_from' && filter.used">
        <CameFrom
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
      ...
      <template v-if="filter.id === 'last_page_title' && filter.used">
        <LastPageTitle
          @apply-filter="applyFilter"
          @remove-filter="removeFilter"
          :filter="filter"
